...
describe("用例测试", () => {
  ...
  test("子包与多级包转换", () =>
    withTmpDir("tmp_subpkg_out", async (dir) => {
      const src_dir = join(dir, "src"),
        out_dir = join(dir, "out");
      mkdirSync(join(src_dir, "sub"), { recursive: true });

      writeFileSync(
        join(src_dir, "root.proto"),
        'syntax="proto3";\npackage test;\nimport "sub/item.proto";\nmessage RootReq { string name = 1; }\nmessage Empty {}\nmessage Call { oneof req { RootReq root = 1; test.sub.ItemReq sub_item = 2; } }\n'
      );
      writeFileSync(
        join(src_dir, "sub", "item.proto"),
        'syntax="proto3";\npackage test.sub;\nmessage ItemReq { uint64 id = 1; }\nmessage Item { uint64 id = 1; }\n'
      );

      gen(join(src_dir, "root.proto"), out_dir, [src_dir]);

      const [root_req_e, item_req_e, call_d] = await Promise.all([
        importDefault(out_dir, "test/RootReqE.js"),
        importDefault(out_dir, "test/sub/ItemReqE.js"),
        importDefault(out_dir, "test/CallD.js")
      ]);

      expect(typeof root_req_e).toBe("function");
      expect(typeof item_req_e).toBe("function");
      expect(typeof call_d).toBe("function");
    }));
