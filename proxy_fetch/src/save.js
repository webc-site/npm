...
const LIMIT_MAX = 3000000,
  ...
  verifyAll = async (to_verify, my_ips) => {
    ...
    for (let i = 0; i < to_verify.length; i += LIMIT_VERIFY_CONCURRENT) {
      ...
      if (bar) {
        ...
        console.log(
          "验证代理 | " + (i + batch.length) + "/" + to_verify.length + " | 成功: " + success_count
        );
      }
    }
  ...
  insert = async (db, verified) => {
    ...
    for (let i = 0; i < verified.length; i += LIMIT_BATCH) {
      ...
      await db.unsafe(
        "INSERT IGNORE INTO proxy(ipv4,port,kind)VALUES" + new_li.join(","),
        args_insert
      );
    }
  };
...
export default async (db, ip_li) => {
  ...
  if (to_verify.length > 0) {
    ...
    const kind_counts = [0, 0, 0];
    for (const [, kind] of verified) {
      kind_counts[kind] += 1;
    }
    kind_stat = " (" + KIND_TO_NAME.map((name, i) => name + ": " + kind_counts[i]).join(", ") + ")";
  }

  const failed_len = to_verify.length - verified_len;
  console.log("有效代理: " + verified_len + kind_stat + " | 失效代理: " + failed_len);
