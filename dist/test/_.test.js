...
mock.module("cersei_rs/logSession", () => {
  return {
    default: (base_url, api_key, model, pkg_path) => {
      let turns = 0;
      return async (prompt) => {
        if (prompt && prompt.includes("is missing a description")) {
          const json_path = join(pkg_path, "package.json");
          if (existsSync(json_path)) {
            const pkg_json = JSON.parse(await read(json_path));
            pkg_json.description = "A mock English description of the package.";
            await writeFile(json_path, JSON.stringify(pkg_json, null, 2) + "\n");
          }
          return;
        }
        turns++;
...
mock.module("@1-/npmver", () => {
  return {
    default: async (pkg) => {
      if (pkg === "test-npmver-pkg") {
        return "1.2.3";
      }
      return undefined;
    }
...
test(
  "指定自定义源码目录执行 prep 测试",
  async () => {
    const [pkg_path] = await setupPrepPkg("tmp_pkg_custom", "test-custom-pkg", "1.0.0"),
      [pkg_name, tmp_dir, , , current_version, next_version] = await prep(
        pkg_path,
        "tmp_pkg_custom",
        "lib"
      ),
...
test(
  "prep 能够基于 npmver 版本号进行累加",
  async () => {
    const [pkg_path] = await setupPrepPkg("tmp_pkg_npmver", "test-npmver-pkg", "1.0.0"),
      [pkg_name, tmp_dir, , , current_version, next_version] = await prep(
        pkg_path,
        "tmp_pkg_npmver_run",
        "lib"
    ...
    expect(pkg_name).toBe("test-npmver-pkg");
    expect(current_version).toBe("1.2.3");
    expect(next_version).toBe("1.2.4");
...
test(
  ...
  async () => {
    const [pkg_path, zh_file] = await setupReadmeGen("tmp_test_readme_gen", {
        name: "test-gen",
        description: "a test package",
        keywords: ["test"]
      }),
      zh_content = await read(zh_file),
      pkg_json = JSON.parse(await read(join(pkg_path, "package.json")));

    expect(zh_content).toContain("A --> B");
    expect(pkg_json.description).toBe("a test package");
...
test(
  "readmeGen 无 description 时自动生成并写入",
  async () => {
    const [pkg_path] = await setupReadmeGen("tmp_test_readme_gen_no_desc", {
        name: "test-gen-no-desc",
        keywords: ["test"]
      }),
      pkg_json = JSON.parse(await read(join(pkg_path, "package.json")));

    expect(pkg_json.description).toBe("A mock English description of the package.");
