...
Omit `directory_path` to validate current working directory.
...
- `src/mdcheck.js`: CLI entry point with configuration loader and hierarchical `.mdcheck.js` search
- `src/scan.js`: Directory scanner using `@1-/walk/walkRelIgnore` with ignore pattern support
- `src/fileValidate.js`: File reader that delegates to markdown validator
- `src/mdValidate.js`: Markdown processor that extracts mermaid blocks and maps errors to original lines
- `src/mermaidValidate.js`: DOM mock injector that provides minimal browser environment for `mermaid.parse()`
