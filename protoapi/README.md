...
- [@1-/protoapi : High-performance binary protocol client for browsers](#1-protoapi-high-performance-binary-protocol-client-for-browsers)
...
ProtoAPI enables high-efficiency, low-overhead communication between web clients and backend services. Core mechanisms include: automatic request batching, automatic captcha challenge retry, unified error status dispatch, and binary data serialization/deserialization. The protocol is Protocol Buffers-style, using varint encoding and UTF-8 string encoding to minimize network payload.
...
ProtoAPI implements request batching via client-side buffering and timer-based flushing. All requests are held in a memory queue and merged into a single HTTP POST after a 1ms delay. Server responses are parsed by ID and status code and dispatched to corresponding Promises.
...
- [@1-/protoapi : 浏览器端高效二进制协议客户端](#1-protoapi-浏览器端高效二进制协议客户端)
  - [功能介绍](#功能介绍)
  - [使用演示](#使用演示)
  - [设计思路](#设计思路)
  - [技术栈](#技术栈)
