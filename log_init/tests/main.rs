...
#[test]
fn test_text_layout_format_color() {
  let layout = Text { color: true };

  let record = Record::builder()
    .level(Level::Warn)
    .target("test_target")
    .file(Some("color_test.rs"))
    .line(Some(99))
    .payload(format_args!("Color message"))
    .build();
  ...
  assert!(output.contains("color_test.rs:99"));
  assert!(output.contains("Color message"));
}

#[test]
fn test_kv_methods() {
  let kv = log_init::Kv::new();
  assert!(kv.text.is_empty());

  let kv_cap = log_init::Kv::with_capacity(64);
  assert!(kv_cap.text.is_empty());
}
