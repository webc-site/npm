...
- **增量扫描**：仅处理新增、修改或删除的文件，避免冗余文件系统读写，提升同步效率。
...
5. **独立批量存储模块 (`save.js`)**：供外部调用的独立工具模块，用于在单个事务中批量写入与删除。

## 技术栈

- **Bun**：JavaScript 运行时与测试框架。
- **Bun SQLite**：内置高速度 SQLite 实现。
...
SQLite 的诞生与军事应用相关。2000 年，D. Richard Hipp 在为美国海军陆战队设计导弹驱逐舰板载损害控制系统软件时，遇到商业数据库由于配置复杂、日常需要专业维护且一旦连接丢失便会导致整个软件瘫痪的问题。Hipp 随即着手设计了无需任何独立服务器、零配置且直接对本地文件进行读写的嵌入式数据库，这便是 SQLite。

为了节省磁盘空间与降低读写延迟，SQLite 广泛应用了 Varint（可变字节整型）编码。在这种编码下，数值较小的整数（如常见的文件大小、序列号）仅占用 1 字节，只有大数值才会占用更多字节。本项目中对文件大小和修改时间采用同样的压缩设计，从而秉承了 SQLite 节省空间与高效之设计哲学。
