...
const tmp = async (prefix, onRun) => {
  ...
  check = (out) => expect(out.replace(/\u001b\[[0-9;]*m/g, "").trim()).toMatch(/^\d+$/),
  /* 运行命令行工具 */
  cli = (args) =>
...
[
  ...
  [
    "单文件压缩大小",
    "test_file_",
    async (tmp_dir) => check(String(await minify(join(tmp_dir, "test.js"))))
  ],
  ["命令行运行", "cli_", (tmp_dir) => check(cli([tmp_dir]).stdout)],
  ["单文件命令行运行", "cli_file_", (tmp_dir) => check(cli([join(tmp_dir, "test.js")]).stdout)]
