...
const CHAR_LF = 10 /* 换行符 \n */,
  ...
  VENDOR_RE = /^-(?:webkit|moz|ms|o)-/,
  isProp = (name) => {
    if (CSS_PROPERTIES.has(name) || name.startsWith("--")) {
      return true;
    }
    if (VENDOR_RE.test(name)) {
      return CSS_PROPERTIES.has(name.replace(VENDOR_RE, "")) || CSS_PROPERTIES.has(name);
    }
    return false;
  },
