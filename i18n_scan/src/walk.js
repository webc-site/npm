...
const save = (map, prefix, rel) => {
  ...
  to_maps: 目标文件 Map 数组
  */
  match = (rel_path, lang_to_idx, from, from_map, to_maps, dir_li, ext_li) => {
...
from: 源语言代码
to_li: 目标语言代码列表
返回值: [from_map, to_maps]
*/
export default async (root, from, to_li, dir_li, ext_li) => {
  const from_map = new Map(),
    to_maps = to_li.map(() => new Map()),
